...
public:
    ...
    virtual void execute() noexcept {
        ...
        // Non-transitive (one-hop) RAPTOR: TRANSITIVE=false separates route and
        // transfer entries, so a transfer can only follow a route leg.
        RAPTOR::RAPTOR<true, RAPTOR::AggregateProfiler, false> algorithm(raptorData);
        const std::vector<StopQuery> queries = generateRandomStopQueries(raptorData.numberOfStops(), n);

        double numJourneys = 0;
        for (const StopQuery& query : queries) {
            algorithm.run(query.source, query.departureTime, query.target, INFTY, maxTransferTravelTime);
            numJourneys += algorithm.getJourneys().size();
        }
        algorithm.getProfiler().printStatistics();
        std::cout << "Avg. journeys: " << String::prettyDouble(numJourneys / n) << std::endl;
