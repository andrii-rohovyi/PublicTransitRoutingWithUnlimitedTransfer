...
// BST Node for storing departure time -> edge indices mapping
struct BSTNode {
    int departureTime;
    std::vector<int> edgeStartIndices;  // startIndex for each edge at this departure time

    BSTNode() : departureTime(0) {}
    BSTNode(int depTime, std::vector<int> indices)
        : departureTime(depTime), edgeStartIndices(std::move(indices)) {}
};

// BST Data for a single vertex - uses std::map as balanced BST
struct BalancedSearchTreeData {
    // Balanced tree: departureTime -> startIndices for all edges
    // std::map is implemented as a Red-Black Tree (balanced BST)
    std::map<int, std::vector<int>> tree;

    // List of edges with trips (in order matching the indices)
    ...
    // Find the entry with departure time >= query time (like bisect_left)
    // Returns iterator to the entry, or end() if none found
    inline std::map<int, std::vector<int>>::const_iterator
    bisectLeft(int departureTime) const noexcept {
        return tree.lower_bound(departureTime);
    }
...
private:
    ...
    std::vector<BalancedSearchTreeData> bstData;

public:
    TimeDependentGraphBST() = default;
    ...
    inline const BalancedSearchTreeData& getBSTData(Vertex u) const noexcept {
        static const BalancedSearchTreeData emptyData;
        if (u >= bstData.size()) return emptyData;
        return bstData[u];
    }

    inline bool hasBSTData(Vertex u) const noexcept {
        return u < bstData.size() && !bstData[u].empty();
    }

    inline static TimeDependentGraphBST FromIntermediate(const Intermediate::Data& inter) noexcept {
        TimeDependentGraphBST tdGraph;
        ...
        std::cout << "Building Balanced Search Tree structures..." << std::flush;
        tdGraph.buildBST();
        std::cout << " done." << std::endl;

        std::cout << "Verifying BST structures..." << std::flush;
        tdGraph.verifyBSTStructure();
        std::cout << " done." << std::endl;

        return tdGraph;
    }

    inline void buildBST() noexcept {
        bstData.resize(graph.numVertices());

        for (size_t i = 0; i < graph.numVertices(); ++i) {
            buildBSTForVertex(Vertex(i));
        }
    }

    inline void verifyBSTStructure() const noexcept {
        size_t errorCount = 0;
        size_t checkedVertices = 0;

        for (size_t u = 0; u < bstData.size(); ++u) {
            const BalancedSearchTreeData& bst = bstData[u];
            if (bst.edges.empty()) continue;

            checkedVertices++;

            // Verify each tree entry
            for (const auto& [depTime, startIndices] : bst.tree) {
                for (size_t edgeIdx = 0; edgeIdx < bst.edges.size(); ++edgeIdx) {
                    ...
                    int startIndex = startIndices[edgeIdx];
                    ...
                    if (startIndex != expectedIndex) {
                        if (errorCount < 10) {
                            std::cout << "\n[BST VERIFY ERROR] Vertex " << u << " edge " << edgeIdx
                                      << " depTime=" << depTime
        ...
        std::cout << "\nBST Verification: checked " << checkedVertices << " vertices, found " << errorCount << " errors" << std::endl;
    }

private:
    inline void buildBSTForVertex(Vertex u) noexcept {
        BalancedSearchTreeData& bst = bstData[u];
        ...
        for (const Edge e : graph.edgesFrom(u)) {
            ...
            if (h.tripCount > 0) {
                ...
                bst.walkingTargets.push_back(v);
                bst.walkingEdges.push_back(e);
            }
        }
        ...
        for (const auto& info : edgeInfos) {
            bst.edges.push_back(info.edgeId);
            bst.targets.push_back(info.target);
        }

        const size_t numEdges = edgeInfos.size();

        // Build the balanced tree: for each unique departure time, compute startIndex for all edges
        for (int depTime : allDepartures) {
            ...
            for (size_t edgeIdx = 0; edgeIdx < numEdges; ++edgeIdx) {
                ...
                startIndices[edgeIdx] = std::distance(departures.begin(), it);
            }

            bst.tree[depTime] = std::move(startIndices);
        }
    }
...
public:
    ...
    inline void printStatistics() const noexcept {
        std::cout << "=== TimeDependentGraphBST Statistics ===" << std::endl;
        std::cout << "Vertices: " << numVertices() << std::endl;
        std::cout << "Edges: " << numEdges() << std::endl;

        size_t totalBSTVertices = 0;
        size_t totalTreeSize = 0;
        size_t totalPositionEntries = 0;
        for (const auto& bst : bstData) {
            if (!bst.empty()) {
                totalBSTVertices++;
                totalTreeSize += bst.tree.size();
                totalPositionEntries += bst.tree.size() * bst.edges.size();
            }
        }

        std::cout << "Vertices with BST data: " << totalBSTVertices << std::endl;
        if (totalBSTVertices > 0) {
            std::cout << "Average tree size per vertex: " << (double)totalTreeSize / totalBSTVertices << std::endl;
