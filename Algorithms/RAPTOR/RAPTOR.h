...
public:
    RAPTOR_prune(const Data& data, const Profiler& profilerTemplate = Profiler()) :
        data(data),
        earliestArrival(data.numberOfStops()),
        stopsUpdatedByRoute(data.numberOfStops()),
        stopsUpdatedByTransfer(data.numberOfStops()),
    ...
    template<typename ATTRIBUTE>
    RAPTOR_prune(const Data& data, const InitialTransferGraph&, const InitialTransferGraph&, const ATTRIBUTE, const Profiler& = Profiler()) :
        RAPTOR_prune(data) {
    }
...
private:
    ...
    inline void relaxTransfers() noexcept {
        stopsUpdatedByTransfer.clear();
        routesServingUpdatedStops.clear();
        const int targetArrivalTime = (TargetPruning && targetStop != noStop)
