...
public:
    ULTRARAPTOR_prune(const Data& data, const InitialTransferType initialTransfers, const Profiler& profilerTemplate = Profiler()) :
        data(data),
        initialTransfers(initialTransfers),
        earliestArrival(data.numberOfStops() + 1),
        stopsUpdatedByRoute(data.numberOfStops() + 1),
    ...
    template<typename ATTRIBUTE>
    ULTRARAPTOR_prune(const Data& data, const InitialTransferGraph& forwardGraph, const InitialTransferGraph& backwardGraph, const ATTRIBUTE weight, const Profiler& profilerTemplate = Profiler()) :
        ULTRARAPTOR_prune(data, InitialTransferType(forwardGraph, backwardGraph, data.numberOfStops(), weight), profilerTemplate) {
    }

    ULTRARAPTOR_prune(const Data& data, const CH::CH& chData, const Profiler& profilerTemplate = Profiler()) requires std::same_as<InitialTransferGraph, CHGraph> :
        ULTRARAPTOR_prune(data, chData.forward, chData.backward, Weight, profilerTemplate) {
    }

    ULTRARAPTOR_prune(const Data& data, const TransferGraph& forwardGraph, const TransferGraph& backwardGraph, const Profiler& profilerTemplate = Profiler()) requires std::same_as<InitialTransferGraph, TransferGraph> :
        ULTRARAPTOR_prune(data, forwardGraph, backwardGraph, TravelTime, profilerTemplate) {
    }
...
private:
    ...
    inline void relaxIntermediateTransfers() noexcept {
        ...
        const int targetArrivalTime = (targetStop != noStop)
                                      ? earliestArrival[targetStop].getArrivalTime()
                                      : never;
        for (const StopId stop : stopsUpdatedByRoute) {
            const int earliestArrivalTime = SeparateRouteAndTransferEntries ? previousRound()[stop].arrivalTime : currentRound()[stop].arrivalTime;
            for (const Edge edge : data.transferGraph.edgesFrom(stop)) {
