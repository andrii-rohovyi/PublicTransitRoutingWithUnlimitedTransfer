...
/**
 * Stop-to-stop delay-tolerant shortcut search.
 *
 * This is the event-level TripBased::DelayShortcutSearch with ONE change:
 * scanCandidateRoutes2() emits stop-to-stop shortcuts into a
 * DynamicTransferGraph instead of event-level shortcuts into a
 * ShortcutCollection. Every other function is identical.
 */
...
public:
    DelayShortcutSearch(const TripBased::Data& tripData, DynamicTransferGraph& shortcutGraph, const int arrivalDelayBuffer, const int departureDelayBuffer) :
        tripData(tripData),
        ...
        witnessReachedIndex1(TripBased::ReachedIndex(tripData)),
        witnessReachedIndex2(TripBased::ReachedIndex(tripData)),
        sourceTimestamp(0),
        originTimestamp(0),
...
private:
    ...
    // *** THE ONLY FUNCTION THAT DIFFERS FROM EVENT-LEVEL ***
    // Instead of: shortcuts.add(originStopEvent, DelayShortcut(destination, travelTime, minOriginDelay, bestMaxOriginDelay));
    // We emit:    stop-to-stop shortcut(origin, destinationStop, travelTime) into shortcutGraph
        ...
        for (const TripId trip : tripsServingUpdatedStops.getKeys()) {
            ...
            for (StopIndex stopIndex(witnessDominatedIndex(trip)); stopIndex != firstCandidateIndex; stopIndex--) {
                ...
                if (arrivalTime <= witnessArrivalTime) {
                    const int sharedOriginMaxDelayVal = getSharedOriginMaxDelay(stop, arrivalTime);
                    minOriginDelayFromTarget = std::min(minOriginDelayFromTarget, sharedOriginMaxDelayVal + 1);
                ...
                if (!shortcutGraph.hasEdge(origin, destinationStop)) {
                    shortcutGraph.addEdge(origin, destinationStop).set(TravelTime, travelTime);
                } else {
                    const Edge edge = shortcutGraph.findEdge(origin, destinationStop);
                    if (shortcutGraph.get(TravelTime, edge) > travelTime) {
                        shortcutGraph.set(TravelTime, edge, travelTime);
                    }
                }
            }
        }
...
private:
    ...
    TripBased::ReachedIndex witnessReachedIndex1;
    TripBased::ReachedIndex witnessReachedIndex2;
    ...
    TripBased::ReachedIndex sharedSourceReachedIndex2;
    std::vector<int> sharedSourceTimestamp;

    TripBased::ReachedIndex witnessDominatedIndex;
